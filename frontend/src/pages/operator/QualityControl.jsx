...
const C = {
  ...
  border: "rgba(64,28,81,0.12)",
};

/* ─────────────────────────────────────────────────────────────
   API fetch helper
   Token is read from localStorage (set by your auth flow).
───────────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────────
   Date Range Picker (unchanged — local UI only, not wired to API)
───────────────────────────────────────────────────────────── */
      ...
      {open && (
        <div className="qc-datepicker__dropdown">
          <div className="qc-datepicker__row">
            ...
            <input
              type="date"
              className="qc-datepicker__input"
          ...
          <button
            type="button"
            className="qc-datepicker__clear"
...
/* ─────────────────────────────────────────────────────────────
   Section views — receive live data from parent
───────────────────────────────────────────────────────────── */
function AcceptanceView({ data }) {
  if (!data) return null;
    ...
    <div className="ma-view">
      ...
      <div className="ma-kpi-row">
        <KpiCard
          ...
          pill="Period"
          sub="Employees used AI suggestion unchanged"
        />
        <KpiCard
          label="Declined (Custom)"
          value={`${kpis.declinedRate}%`}
          pill="Period"
          sub="Employee edited before submitting"
          flag={kpis.declinedRate > 40 ? "warn" : undefined}
        />
        <KpiCard
          label="Rejected (No Action)"
          value="0%"
          pill="Period"
          sub="No separate rejected status — see declined_custom"
          flag="neutral"
        />
        <KpiCard
          ...
          pill="Period"
          sub="Feedback rows in ticket_resolution_feedback"
        />
      </div>
      ...
      <div className="ma-cards-row">
        <Card
          ...
          subtitle="Daily accepted vs declined_custom. A week-long decline in acceptance is a model signal."
        >
            ...
            {(breakdown.accepted + breakdown.declined) === 0 ? <EmptyState /> : (
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
                  <Pie
                    data={[
                      { label: "Accepted",         value: breakdown.accepted },
                      { label: "Declined (Custom)", value: breakdown.declined },
                    ]}
  ...
  return (
    <div className="ma-view">
      ...
      <div className="ma-kpi-row">
        <KpiCard
          label="Rescore Rate"
          value={`${kpis.rescoreRate}%`}
          pill="vs model_priority"
          sub="Employees changed AI-assigned priority"
          flag={kpis.rescoreRate > 15 ? "warn" : undefined}
        />
      ...
      <Card
        title="Rescore Direction by Department"
        subtitle="Upscores vs downscores per department. Consistent upscoring = model undertriages that team's tickets."
        wide
      >
        <div className="ma-chart-box">
...
function ReroutingView({ data }) {
  ...
  return (
    <div className="ma-view">
      ...
      <div className="ma-cards-row">
        <Card
          title="Reassignment Rate by Department"
          subtitle="Avg rerouting requests per ticket. Amber ≥ 0.5, red ≥ 1. High avg = DepartmentRoutingAgent miscalibrated."
        >
              ...
              <table className="ma-table">
                ...
                <tbody>
                  {reviewCases.slice(0, 20).map((c) => {
                    const reason = c.humanOverridden && c.wasRescored
                      ? "Override + Rescore"
                      : c.humanOverridden
                      ? "Override"
                      : "Rescore";
                    return (
                      <tr key={c.ticketId}>
                        ...
                        <td className="ma-td--muted">
                          {c.createdAt ? new Date(c.createdAt).toLocaleDateString("en-GB") : "—"}
                        </td>
                        <td>{c.department}</td>
                        <td>
                          {c.modelPriority && c.modelPriority !== c.priority
                            ? <span className="ma-route-text">{c.modelPriority} → {c.priority}</span>
                            : <span>{c.priority}</span>
                          }
                        </td>
                        <td>
                          <span className={`ma-reason-pill ma-reason-pill--${
                            reason.includes("Override") ? "amber" : "blue"
                          }`}>
                            {reason}
                          </span>
                        </td>
                        <td>
                          <button
                            ...
                            type="button"
                            onClick={() => navigate(`/operator/complaints/${c.ticketCode}`)}
                          >
                            <span className="ma-open-btn__icon">↗</span>  
                            Open
                          </button>
                        </td>
                      </tr>
...
───────────────────────────────────────────────────────────── */
const QC_SECTIONS = [
  { id: "acceptance", label: "A — Acceptance", icon: "🎯" },
  { id: "rescoring",  label: "B — Rescoring",  icon: "⚖️" },
  { id: "rerouting",  label: "C — Rerouting",  icon: "🔀" },
];
...
export default function QualityControl() {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await apiFetch("/operator/analytics/qc", {
  ...
  return (
    <Layout role="operator">
      <div className="modelAnalysis" ref={revealRef}>
        ...
        {/* Content */}
        {!loading && !error && qcData && (
          <>
            {activeSection === "acceptance" && <AcceptanceView data={qcData.acceptance} />}
            {activeSection === "rescoring"  && <RescoringView  data={qcData.rescoring}  />}
            {activeSection === "rerouting"  && <ReroutingView  data={qcData.rerouting}  />}
          </>
        )}
      </div>
    </Layout>
