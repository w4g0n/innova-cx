...
SELECT ms.email, crypt('Innova@2025', gen_salt('bf')), 'manager'::user_role, TRUE
...
SELECT u.id, ms.full_name, ms.department_id, ms.employee_code, 'Department Manager'
FROM manager_seed ms
...
INSERT INTO users (email, password_hash, role, is_active)
SELECT es.email, crypt('Innova@2025', gen_salt('bf')), 'employee'::user_role, TRUE
...
INSERT INTO user_profiles (user_id, full_name, department_id, employee_code, job_title)
SELECT u.id, es.full_name, es.department_id, es.employee_code, 'Support Specialist'
FROM employee_seed es
